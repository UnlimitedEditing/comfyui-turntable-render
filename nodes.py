...
def _render_one(scene: pyrender.Scene,
    ...
    # Camera: ~24° vertical FOV ≈ 85 mm equivalent
    cam = pyrender.PerspectiveCamera(yfov=0.42, znear=0.01, zfar=1000.0)

    # Key light: follows camera for even front illumination
    dl_key = pyrender.DirectionalLight(color=np.ones(3), intensity=3.0)

    # Fill light: fixed position slightly above and to the side of the scene
    # so back views aren't pitch-black
    fill_eye  = center + np.array([radius * 2.0, radius * 3.0, radius * 0.5])
    fill_pose = _look_at_pose(fill_eye, center)
    dl_fill   = pyrender.DirectionalLight(color=np.ones(3), intensity=1.2)
...
class TurntableRenderNode:
    ...
    def render(self, model_path, num_views, elevation_deg, frame_w, frame_h):
